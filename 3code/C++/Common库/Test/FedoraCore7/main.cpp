...
int CTestSerial::run(void)
{
    ...
    CSerialExt serialExt1("/dev/ttyS0");
    CSerialExt serialExt2("/dev/ttyUSB0");
...
{    
	...
	CRingBuffer m_queueBuffer;
	char pStr[100];
	
	if(!m_queueBuffer.allocBuffer(12))
    {
		ASSERT(false);
    }

    printf("test 1: diff > 0, diff1 > size\n");
	if(m_queueBuffer.put("ABCDEFGHI", 10, false))
	{
		int i;
		if(m_queueBuffer.get(pStr, 10))
		{
			printf("get:%s\n", pStr);
		}
	    m_queueBuffer.put("JKLM", 5, false);
	    int getSize = m_queueBuffer.put("NOPQ", 5, true);
		getSize = m_queueBuffer.get(pStr, getSize);
		printf("get:");
		for(i = 0; i < getSize; i++)
		{
			printf("%c", pStr[i]);
	...
	printf("test 2: diff > 0, diff1 <= size\n");
    if(m_queueBuffer.put("ABCDEFGHI", 10, false))
    {
		int i;
		if(m_queueBuffer.get(pStr, 5))
		{
			printf("get:%s\n", pStr);
		}
		m_queueBuffer.put("JKLM", 5, false);
	...
	if(!m_queueBuffer.allocBuffer(15))
    {
		ASSERT(false);
    }
	printf("test 3: diff <= 0, diff1 > size\n");
    if(m_queueBuffer.put("ABCDEFGHIJLMNO", 15, false))
    {
		int i;
		int getSize = m_queueBuffer.get(pStr, 10);
		printf("get:");
		for(i = 0; i < getSize; i++)
		{
			printf("%c", pStr[i]);
		...
		getSize = m_queueBuffer.put("PQRS", 5, true);	
		getSize = m_queueBuffer.get(pStr, getSize);
		printf("get:");
		for(i = 0; i < getSize; i++)
		{
	...
	printf("test 4: diff <= 0, diff1 <= size\n");
	if(m_queueBuffer.put("ABCDEFGHI", 10, false))
	{
		int getSize = m_queueBuffer.put("JKLMN", 6, true);	
		if(m_queueBuffer.get(pStr, getSize))
		{
			int i;
			for(i = 0; i < getSize; i++)
			    printf("%c", pStr[i]);
