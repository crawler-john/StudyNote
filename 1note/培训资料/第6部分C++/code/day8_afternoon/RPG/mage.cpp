...
//构造函数
//法师的血量为70,魔法量为150,攻击力为20,防御力为2
Mage::Mage(const string &t_name, Level t_level)
    :Player(t_name,t_level,MagiCian,roleData(70,150,20,2))
{
    //装备为魔法杖,基础装备
    m_equip = Equip(g_equipName[3],BasicEquip);
    m_skill = Skill(g_skillName[3],SpecialSkill);
}

void Mage::basisAttack()
{
    //如果被攻击的目标血量为0
    if(m_enemy->getHP() <= 0)
    {
...
void Mage::magicAttack(const Skill &t_skill)
{
